---
name: skill-router
description: Catalog, install, remove, and sync Claude Code skills via the Synapse /skills proxy. Skills are plain directories under .claude/skills/ — no git submodules, no GitHub auth on the client. Use whenever the user asks which skills exist, wants to install/fetch/sync/remove a skill, or starts a task that would benefit from a registered skill that is not yet installed locally. Trigger proactively when a request maps to a known skill (e.g. notes, tenants, tags, deploys, MCP servers, rules) and that skill is not yet under .claude/skills/.
---

# Skill Router

Manages the set of skills installed under `.claude/skills/`. Every skill is fetched from the **Synapse `/skills` proxy**, which mirrors the private skill-* GitHub repos owned by the authoring organisation. The router does three things:

1. Lists the available catalog by calling `GET {SYNAPSE}/skills`.
2. Installs a skill by downloading `GET {SYNAPSE}/skills/<name>/download` (a `.skill` zip) and unpacking it into `.claude/skills/<name>/`.
3. Tells Claude, via its description, which skills exist so the router can fetch them on demand.

## Why Synapse as the source of truth

- Clients don't need a GitHub token. Synapse holds the GitHub App credentials and proxies the bytes.
- Skill repos can stay private. Only Synapse needs read access.
...
- `SYNAPSE_URL` — base URL of the Synapse API. Defaults to `https://synapse.tri2b.cloud`. Override in local/dev environments.
...
| `scripts/install.sh <name>` | Downloads `<name>.skill` from Synapse and unpacks it to `.claude/skills/<name>/`. Refuses if that path already exists. |
...
- Sync always pulls the **latest release**; pin by not running sync, or use a specific Synapse instance that lags behind production.

## Caveats

- `curl`, `unzip`, and `jq` are required on the host for the scripts to work. All three are standard on devbox/macOS/Linux.
- Synapse must be reachable. If your environment can't hit `SYNAPSE_URL`, set the env var to a reachable instance (e.g. a port-forward).
- If a skill doesn't appear in `catalog.sh`, it is either not published yet, not tagged with the `claude-skill` topic in its GitHub repo, or the Synapse GitHub App doesn't have access to it.
